...
public class UiOfLoginPageForChinese extends BaseClass
{ 
    ...
    public void selectChinese() {
    	...
    	// ✅ Select Chinese by visible text
    	select.selectByVisibleText("中文");

    	// Verify Chinese is selected
    	String selectedText = select.getFirstSelectedOption().getText();
    	Assert.assertEquals(selectedText, "中文", "❌ Failed to select '中文' from dropdown.");
    ...
    public void signIn() 
    {
        ...
        Assert.assertEquals(labelText, "登入", "Label text is not matching!");
    }
    ...
    public void inputBox() 
    {
        ...
        Assert.assertEquals(plcHolder, "用戶名/電子郵件/手機", "Placeholder text does not match expected.");
    ...
    public void toggleButton() throws InterruptedException 
    {
     ...
     // Assert that the disabled toggle button is actually enabled
      ...
      //  Thread.sleep(2000);
        ...
        Assert.assertEquals(labelText, "记住我", "Label text is not matching!");
    }

    @Test(priority = 8)
    public void nextBtnBackBtn() throws InterruptedException 
    {
        WebElement nxtbtn = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='signinNxt']")));
        String labelText = nxtbtn.getAttribute("value");
        System.out.println("Chinese Next Button :" + labelText );
        Assert.assertEquals(labelText, "下一个", "Label text is not matching!");
    ...
    public void secureLoginBackBtn() throws InterruptedException 
    {
        ...
        Assert.assertEquals(labelText, "通過安全登錄", "Label text is not matching!");

        WebElement app = driver.findElement(By.xpath("//a[@id='loginVia_App']"));
        String labelText2 = app.getText();
	    Assert.assertEquals(labelText2, "应用", "Label text is not matching!");
