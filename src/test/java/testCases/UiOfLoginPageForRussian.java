...
//
//public class UiOfLoginPageForRussian extends BaseClass{ 
//    
//  @Test(priority = 1)
...
//  	Select select = new Select(langDropdown);
//
//  	// ✅ Select русский by visible text
//  	select.selectByVisibleText("русский");
//
//  	// Verify русский is selected
//  	String selectedText = select.getFirstSelectedOption().getText();
//  	Assert.assertEquals(selectedText, "русский", "❌ Failed to select 'русский' from dropdown.");
...
//      Assert.assertEquals(labelText, "Войти", "Label text is not matching!");
//  }
//
//  @Test(priority = 5)
...
//      Assert.assertEquals(plcHolder, "Имя пользователя/Электронная почта/Мобильный телефон", "Placeholder text does not match expected.");
//
...
//      Assert.assertEquals(labelText, "Запомнить меня", "Label text is not matching!");
//  }
//
//  @Test(priority = 8)
...
//      
//      Assert.assertEquals(labelText, "Далее", "Label text is not matching!");
//      
//      nxtbtn.click();
//      Thread.sleep(2000);
...
//      Assert.assertEquals(labelText, "Безопасный вход через", "Label text is not matching!");
//
//      WebElement app = driver.findElement(By.xpath("//a[@id='loginVia_App']"));
//      String labelText2 = app.getText();
//	    Assert.assertEquals(labelText2, "апп", "Label text is not matching!");
//     
//      
//      app.click();
//      
